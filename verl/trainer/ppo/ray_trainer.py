...
def apply_invalid_action_penalty(data: DataProto, invalid_action_penalty_coef=float):
    ...
    valid_action_ratios = []
    action_format_penalties = []
    action_response_clip_rates = []
    action_missing_action_rates = []
    action_missing_think_close_rates = []
    action_repetition_rates = []
    for i in range(len(data)):
        ...
        if 'invalid_action_count' in data_item.non_tensor_batch:
            invalid_count = np.asarray(data_item.non_tensor_batch['invalid_action_count']).astype(np.float32).sum()
            action_invalids = torch.tensor(invalid_count, dtype=torch.float32, device=prompt_ids.device)
            if 'trajectory_valid_action_ratio' in data_item.non_tensor_batch:
                valid_action_ratios.append(float(np.asarray(data_item.non_tensor_batch['trajectory_valid_action_ratio']).astype(np.float32).mean()))
        else:
            ...
            valid_action_ratios.append(float(np.asarray(action_valids).astype(np.float32).mean()))
        # invalid action penalty
        # assert reward_tensor[i, valid_response_length - 1] != 0.0, f'i={i}'
        reward_tensor[i, reward_index] -= invalid_action_penalty_coef * action_invalids

        if 'step_rewards' in data.batch.keys():
            step_rewards[i] -= invalid_action_penalty_coef * action_invalids

        action_format_penalty = 0.0
        if 'action_format_penalty_sum' in data_item.non_tensor_batch:
            action_format_penalty = float(np.asarray(data_item.non_tensor_batch['action_format_penalty_sum']).astype(np.float32).sum())
        elif 'action_format_penalty' in data_item.non_tensor_batch:
            action_format_penalty = float(np.asarray(data_item.non_tensor_batch['action_format_penalty']).astype(np.float32).sum())
        if action_format_penalty:
            penalty_tensor = torch.tensor(action_format_penalty, dtype=torch.float32, device=prompt_ids.device)
            reward_tensor[i, reward_index] += penalty_tensor
            if 'step_rewards' in data.batch.keys():
                step_rewards[i] += penalty_tensor
        action_format_penalties.append(action_format_penalty)

        action_count = 1.0
        if 'trajectory_action_count' in data_item.non_tensor_batch:
            action_count = max(float(np.asarray(data_item.non_tensor_batch['trajectory_action_count']).astype(np.float32).sum()), 1.0)

        def _rate(count_key: str, flag_key: str) -> float:
            if count_key in data_item.non_tensor_batch:
                return float(np.asarray(data_item.non_tensor_batch[count_key]).astype(np.float32).sum()) / action_count
            if flag_key in data_item.non_tensor_batch:
                return float(np.asarray(data_item.non_tensor_batch[flag_key]).astype(np.float32).mean())
            return 0.0

        action_response_clip_rates.append(_rate('action_response_clip_count', 'action_response_clipped'))
        action_missing_action_rates.append(_rate('action_missing_action_count', 'action_missing_action'))
        action_missing_think_close_rates.append(_rate('action_missing_think_close_count', 'action_missing_think_close'))
        action_repetition_rates.append(_rate('action_repetition_count', 'action_repetition'))
    
    if valid_action_ratios:
        valid_action_ratio = float(np.mean(valid_action_ratios))
    else:
    ...
    if action_format_penalties:
        metrics.update({
            'episode/action_format_penalty_mean': float(np.mean(action_format_penalties)),
            'episode/action_response_clip_rate': float(np.mean(action_response_clip_rates)),
            'episode/action_missing_action_rate': float(np.mean(action_missing_action_rates)),
            'episode/action_missing_think_close_rate': float(np.mean(action_missing_think_close_rates)),
            'episode/action_repetition_rate': float(np.mean(action_repetition_rates)),
        })
    return data, metrics
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if multi_turn:
            # Trajectory-level multi-turn rollouts carry loss_mask because a
            # single sample interleaves trainable assistant spans with
            # non-trainable user/tool spans. Vanilla turn-level rollouts are
            # already one generated response per sample and do not carry
            # loss_mask, so response_mask is the correct fallback.
            if "loss_mask" in data.batch:
                response_length = grpo_calculation_mask.size(1)
                grpo_calculation_mask = data.batch["loss_mask"][:, -response_length:]

        is_skill_turn = data.non_tensor_batch.get("is_skill_management_turn", None)
        if is_skill_turn is None:
            ...
            # Turn-level skill-management samples have a different reward semantics
            # from environment action samples. Normalize them in separate GRPO
            # groups, then merge the token-level advantages back into one batch.
            is_skill_turn = np.array([
                bool(np.asarray(value).reshape(-1)[0]) if np.asarray(value).size > 0 else False
                for value in is_skill_turn
            ], dtype=bool)
            advantages = torch.zeros_like(data.batch["token_level_rewards"])
            returns = torch.zeros_like(data.batch["token_level_rewards"])

            def _fill_grpo_advantage(sample_mask: np.ndarray, weight: float = 1.0):
                if not np.any(sample_mask):
                    return
                sample_mask_t = torch.as_tensor(sample_mask, dtype=torch.bool, device=data.batch["token_level_rewards"].device)
                subset_advantages, subset_returns = core_algos.compute_grpo_outcome_advantage(
                    token_level_rewards=data.batch["token_level_rewards"][sample_mask_t],
                    response_mask=grpo_calculation_mask[sample_mask_t],
                    index=data.non_tensor_batch["uid"][sample_mask],
                    traj_index=data.non_tensor_batch['traj_uid'][sample_mask],
                    norm_adv_by_std_in_grpo=norm_adv_by_std_in_grpo,
                )
                advantages[sample_mask_t] = subset_advantages * weight
                returns[sample_mask_t] = subset_returns * weight

            _fill_grpo_advantage(~is_skill_turn)
            _fill_grpo_advantage(
                is_skill_turn,
                weight=float(kwargs.get("skill_mgmt_advantage_weight", 1.0)),
            )
...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for val_batch_idx, test_data in enumerate(self.val_dataloader, start=1):
            print(f"validation generation start: batch {val_batch_idx}/{total_val_batches}")
            ...
            print(f"validation generation end: batch {val_batch_idx}/{total_val_batches}")
            del test_batch
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            skill_mgmt_advantage_weight=self.config.env.get("skill_tool_rollout", {}).get("skill_mgmt_advantage_weight", 1.0),
                        )
