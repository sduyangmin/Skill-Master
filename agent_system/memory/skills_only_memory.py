    ...
    * ``"embedding"`` – the task description is encoded with a
    ...
    def _detect_task_type(self, task_description: str) -> str:
        ...
        if 'pick_and_place' in task_specific or 'clean' in task_specific:
            # Keep the routing conservative for ALFWorld: most natural-language
            # goals describe object transport, while only a narrow subset are
            # true observation tasks.  Over-routing generic "find ... and put
            # ..." goals into ``examine`` pollutes retrieval with lamp-specific
            # skills (for example desklamp skills on ordinary placement tasks).
            if 'look at' in goal and 'under' in goal:
                return 'look_at_obj_in_light'
            elif 'clean' in goal:
                return 'clean'
            elif 'heat' in goal or 'heated' in goal or 'hot' in goal:
                return 'heat'
            elif 'cool' in goal or 'cooled' in goal or 'cold' in goal:
                return 'cool'
            elif (
                'examine' in goal and
                any(token in goal for token in ('lamp', 'desklamp', 'floorlamp'))
            ):
                return 'examine'
            else:
        ...
        **kwargs,
        ...
        Returns:
            Dictionary with keys:
              ...
              - ``task_type``            – provided or detected task type string
        ...
        if self.retrieval_mode == "embedding":
            ...
            # Still keep task type for bookkeeping / formatting labels.
            task_type = task_type or self._detect_task_type(task_description)
            return {
