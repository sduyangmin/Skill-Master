...
class WebshopWorker:
    ...
    def step(self, action):
        ...
        try:
            ...
            if session is not None and str(session).isdigit():
                goal_idx = int(session)
            goal = self.env.server.user_sessions[session]['goal']
        except Exception:
            pass
    ...
    def reset(self, idx):
        ...
        info['available_actions'] = self.env.get_available_actions()
        info['won'] = False
        info['goal_idx'] = int(idx)
        try:
            info['goal'] = self.env.server.user_sessions[self.env.session]['goal']
        except Exception:
            info['goal'] = None
        return obs, info
            ...
            if isinstance(kwargs, (list, tuple)):
                for item in kwargs:
                    ...
                    if isinstance(item, dict):
                        goal_idx = item.get('goal_idx', None)
                    else:
                        goal_idx = item
                    if goal_idx is None:
                        parsed_idxs = []
                        break
                    try:
                        parsed_idxs.append(int(goal_idx))
                    except (TypeError, ValueError):
                        parsed_idxs = []
                        break
            if len(parsed_idxs) == self.num_processes:
                idx = parsed_idxs
