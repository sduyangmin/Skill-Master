...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs):
        ...
        if self.retrieval_memory is not None:
            ...
            use_skills_only_memory = self.config.env.get('use_skills_only_memory', False)
            for i, task in enumerate(self.tasks):
                retrieve_kwargs = {}
                if use_skills_only_memory:
                    gamefile = self.gamefile[i] if i < len(self.gamefile) else None
                    retrieve_kwargs["task_type"] = infer_alfworld_skill_category(task, gamefile=gamefile)
                memories = self.retrieval_memory.retrieve(
                    task_description=task,
                    top_k=mem_config.get('top_k', 10),
                    similarity_threshold=mem_config.get('similarity_threshold', 0.7),
    ...
    def build_text_obs(self, text_obs: List[str], admissible_actions: List[List[str]], init: bool = False) -> List[str]:
        ...
        for i in range(len(text_obs)):
            ...
            retrieval_on_init = self.config.env.get('retrieve_memory_on_init', False)
            if self.config.env.get('use_skills_only_memory', False):
                retrieval_on_init = retrieval_on_init or self.config.env.skills_only_memory.get('retrieve_on_init', False)
            elif self.config.env.get('use_retrieval_memory', False):
                retrieval_on_init = retrieval_on_init or self.config.env.retrieval_memory.get('retrieve_on_init', False)

            # Use retrieval memory template if enabled
            use_retrieval = (self.retrieval_memory is not None and
                           self.retrieved_memories is not None and
                           (not init or retrieval_on_init))
...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs) -> Dict[str, Any]:
        ...
        self.goal_idx = [info.get('goal_idx') for info in infos]
        self.goals = [info.get('goal') for info in infos]
