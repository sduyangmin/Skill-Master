...
# - BASE_SKILL_BANK_PATH and SKILL_BANK_PATH control the initial and mutable
#   skill banks. Set RESET_SKILL_BANK=1 to reinitialize the mutable bank.
# - Optional overrides can be supplied via environment variables below or via
#   trailing Hydra arguments after the script invocation.
...
export WANDB_NAME=${WANDB_NAME:-webshop_grpo_turnlevel_skill_tool}
...
base_skill_bank_path=${BASE_SKILL_BANK_PATH:-memory_data/webshop/claude_style_skills_minimal.json}
skill_bank_path=${SKILL_BANK_PATH:-memory_data/webshop/self_managed_skills.json}
...
# Trainer schedule
experiment_name=${EXPERIMENT_NAME:-webshop_grpo_turnlevel_skill_tool}
save_freq=${SAVE_FREQ:-5}
test_freq=${TEST_FREQ:-5}
total_epochs=${TOTAL_EPOCHS:-300}

if [[ "${RESET_SKILL_BANK:-0}" == "1" ]]; then
    rm -f "$skill_bank_path"
fi
